...
R"HTML(<header><h1>&#x1F4A7; Water Logger <span class="badge">SETUP MODE</span></h1><div class="sub">Upload UI files to /www/ to restore normal operation &mdash; or bookmark <strong>/setup</strong> for recovery</div><div id="fs-sysinfo" class="sub" style="font-size:.72rem;margin-top:3px;opacity:.8"></div></header><div class="container"><div class="tabs"><button type="button" class="tab active" id="tab-btn-setup" onclick="switchTab('setup',this)" aria-label="Setup tab">&#x2699;&#xFE0F; Setup</button><button type="button" class="tab" id="tab-btn-corelogic" onclick="switchTab('corelogic',this)" aria-label="Core Logic tab">&#x1F9E9; Core Logic</button></div>)HTML"
