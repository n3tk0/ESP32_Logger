...
fetch('/api/status').then(function(r){return r.json()}).then(function(d){var el=document.getElementById('fs-sysinfo');if(!el)return;var parts=[];if(d.version)parts.push('FW\u00a0'+d.version);if(d.fsUsed!=null&&d.fsTotal)parts.push('LittleFS\u00a0'+fmB(d.fsUsed)+'\u00a0/\u00a0'+fmB(d.fsTotal));if(parts.length)el.textContent=parts.join('\u2003\u00b7\u2003')}).catch(function(){});
