...
| 1.10 | M | RtcManager.cpp:27-28 — `delete rtcWire` before `delete Rtc`. Rtc dtor may dereference dangling _wire reference. | Swap order: `delete Rtc; Rtc = nullptr; delete rtcWire; rtcWire = nullptr;` (delete BEFORE nulling, else leak). | Fixed (R23 PR pending) |
| 1.11 | M | RtcManager.cpp:71-77 — `backupBootCount` writes magic byte FIRST then bootcount bytes. Partial write leaves valid magic + bad value. | Write 4 bootcount bytes first, then magic last. Reverse order in backupBootCount. | Fixed (R23 PR pending) |
| 1.12 | L | RtcManager.cpp:46 — `RtcDateTime(__DATE__, __TIME__)` fallback regresses wall clock on every reflash. | Use 2024-01-01 00:00:00 baseline; surface "time invalid" in UI. | Fixed (R23 PR pending) |
...
| 2.12 | M | DataPipeline.h:101-124 `collectMetricSeries` self-aliasing memmove pattern. Source/dest overlap when count > maxOut/2. | Use `memmove(out, out+maxOut-count, count*sizeof(float));` | Fixed (R26 PR pending) |
| 2.13 | M | HardwareManager.cpp:27 — `pulseCount++` in ISR is not atomic on RV32. | `__atomic_fetch_add(&pulseCount, 1, __ATOMIC_RELAXED);` | Fixed (R28 PR pending) |
| 2.14 | M | TaskManager.cpp:193-209 — `hb == 0` treated as "not started" indefinitely; deadlocked task at line 16 never tripped. | Seed each heartbeat slot in init() with `millis()`; remove the `hb==0` exemption. | Fixed (R23 PR pending) |
...
| 2.16 | M | TaskManager.cpp:115-126 — Mirror mode holds fsMutex for primary + mirror appendRow. SD writes block 50-100ms. | Release fsMutex between primary and mirror, or use dedicated mirrorFsMutex. | Fixed (R26 PR pending) |
...
| 2.18 | L | ExportTask.cpp:50 — Shutdown flush calls `exportManager.sendAll` (blocking TLS HTTP) inside task-exit path. ESP.restart can race. | Skip final flush on shutdown (`if (running && batchCount > 0)`), or cap to 1s. | Fixed (R26 PR pending) |
| 2.19 | M | AlertEngine.evaluate called from ProcessingTask.cpp:57 with no caller-side mutex. `handleApiAlertsSave` mutates rules concurrently. | AlertEngine must take its internal `_mutex` in evaluate() (verify in Phase 15). | Fixed (R26 PR pending) |
...
| 3.10 | M | ApiHandlers.cpp:53-58 — sensorFilter/metricFilter c_str() captured but inconsistent with the lifetime fix applied to agg/mode at line 106-112. | Apply the same copy-to-local-buffer pattern. | Fixed (R26 PR pending) |
| 3.11 | M | ApiHandlers.cpp:611-647 wifi-test — globals updated without atomic_thread_fence. Reader can see WT_SUCCESS with stale ip/rssi. | `std::atomic<WifiTestState> g_wtState;` with release/acquire. | Fixed (R26 PR pending) |
...
| 3.16 | M | ApiHandlers.cpp:488-493 `/api/ota/rollback` — `delay(200)` on AsyncTCP worker. | Use deferred flag like NTP sync pattern. | Fixed (R26 PR pending) |
| 3.17 | M | WebServer.cpp:1289-1330 `/set_time` — ~120ms of `delay()` in RTC writes from AsyncTCP worker. | Move RTC mutation to loop() deferred handler. | Fixed (R26 PR pending) |
| 3.18 | M | WebServer.cpp:1422 `/download` TOCTOU between exists() and beginResponse(). | Null-check `resp` before `addHeader`. | Fixed (R26 PR pending) |
...
| 3.20 | M | ApiHandlers.cpp:807-873 `/api/backup` — takes configMutex but inhaleJsonFile reads files without fsMutex. | Acquire fsMutex around each `inhaleJsonFile` call. | Fixed (R26 PR pending) |
...
| 4.2 | M | sensors.js:104 — reads `d.last_sweep_ms` which backend never emits. | Emit `last_sweep_ms` from SensorManager::toJson; OR remove the UI label. | Fixed (R24 PR pending) |
...
| 4.9 | M | settings.js:1404 otaUpload XHR — no timeout. Mid-flash crash hangs UI. | `xhr.timeout = 120000; xhr.ontimeout = function() { ... }` | Fixed (R24 PR pending) |
| 4.10 | M | pages.js:1000 `/api/live` polling — no in-flight guard. Backend stall queues many requests. | Track `liveInFlight`; skip tick if previous unfinished. | Fixed (R24 PR pending) |
| 4.11 | M | pages.js:963-980 EventSource — no client-side keepalive. Silent stall not detected. | Reset a 10s timer on each event; on timeout close+reopen. | Fixed (R24 PR pending) |
| 4.12 | M | core.js:438-451 CSRF cache — only `settingsSave` retries on 403. Other call sites do not. | Centralize: every mutating fetch uses a `postWithCsrf()` helper. | Fixed (R24 PR pending) |
...
| 4.16 | L | pages.js:686 — filelist `truncated` flag from backend never read in UI. >500 files: silent partial list. | Render "showing first 500" banner when `d.truncated`. | Fixed (R24 PR pending) |
...
| 6.8 | M | IModule.h:42-43 — Asymmetric error reporting: `load()` returns bool, `save()` returns void. JSON capacity overflow silently truncates. | `virtual bool save(JsonObject cfg) const = 0;` and propagate to ModuleRegistry::saveAll. | Fixed (R23 PR pending) |
...
| 6.10 | L | IModule.h:75 — schema() return is unvalidated; malformed JSON silently breaks the UI (settings.js:1702). | Add `bool validateSchema(const char* s)` helper called at registration time in `ModuleRegistry::add`. | Fixed (R23 PR pending) |
...
| 6.13 | M | ModuleRegistry.cpp:63-67 — Parse error leaves corrupt modules.json untouched; subsequent boots load defaults silently forever. | Quarantine: `fs.rename(path, "/config/modules.json.corrupt"); saveAll(fs, path);` | Fixed (R23 PR pending) |
...
| 6.15 | M | ModuleRegistry.cpp:36-44 + 84-127 — saveAll does not take fsMutex; loadAll's crash-recovery rename also unlocked. | Acquire `fsMutex` at function entry of saveAll; expose `MutexGuard` helper for symmetry. | Fixed (R23 PR pending) |
| 6.16 | L | ModuleRegistry.cpp:116-120 comment promises atomic rename on LittleFS but the class accepts any `fs::FS&`. SD/FAT rename-over-existing fails. | Add probe: if `fs.exists(path)` → `fs.remove(path)` first when target FS != LittleFS. | Fixed (R23 PR pending) |
| 6.17 | M | ModuleRegistry.cpp:55-60 — Oversize-file rejection returns false but doesn't quarantine. File stays oversize across reboots. | Same fix as 6.13. | Fixed (R23 PR pending) |
...
| 7.2 | L | Utils.cpp:30 — control-char filter only checks `< 0x20` and `0x7f`. Bytes 0x80-0xFF (UTF-8 lead bytes, non-ASCII) pass. Not a security issue but allows weird filenames. | Document as design choice or restrict to ASCII printable. | Fixed (R26 PR pending) |
| 7.3 | L | Utils.cpp:57 `sanitizeFilename` 64-char cap. Generated datalog filenames (prefix + device id + timestamp + .txt) can approach 50-60 chars; close margin. | Bump to 96 chars; verify all generator paths against new limit. | Fixed (R26 PR pending) |
| 7.4 | M | Utils.cpp:83-134 `deleteRecursive` uses heap `std::vector` work-stack with no size cap. A crafted/corrupt FS tree with many directories can OOM the AsyncTCP worker. | Add `if (stack.size() > 256) return false;` to abort deep recursion. | Fixed (R26 PR pending) |
...
| 8.7 | M | RtcManager.cpp:93-96 `restoreBootCount` — discards `f.read()` return value. On short file (truncated by 8.6 power-loss scenario), partial overwrite of `bootCount`; remaining bytes keep old stack/BSS state. | `size_t n = f.read(...); if (n != sizeof(bootCount)) bootCount = 0;` | Fixed (R23 PR pending) |
...
| 8.9 | L | RtcManager.cpp:212-215 — ESP32/S2/S3 EXT1 wake path silently `return` when wakeupMode != ACTIVE_HIGH. User configures ACTIVE_LOW → device never wakes from GPIO. No status reported. | Set `statusMessage = "EXT1 wake requires ACTIVE_HIGH on this chip"` so the UI surfaces it. | Fixed (R23 PR pending) |
...
| 9.5 | L | ConfigManager.cpp:352 `migrateConfig` ends with `saveConfig()` during setup() before fsMutex exists. Today benign (single-threaded). If migrate is ever called outside setup, race. | Add comment "migrate must run pre-task-init". | Fixed (R23 PR pending) |
...
| 10.3 | L | SensorTask.cpp:28, SlowSensorTask.cpp:25 — fallback `ts = millis()/1000` returns 0 for the first second of uptime. SensorTypes.h:21 reserves `ts=0` as "unknown" and LiveAggregator uses `_lastFlushEpoch==0` as "first call" sentinel. Collision during first 1 s after boot. | Bump: `ts = (millis()/1000UL) + 1;` to avoid 0 in the fallback path. | Fixed (R26 PR pending) |
| 10.4 | L | SlowSensorTask.cpp:31 — Hardcoded 500 ms outer poll. Not configurable, not data-driven. For SDS011 with 60-90 s internal cycles, 180 wakeups per useful read. Minor energy waste. | Compute from slow-sensor min interval, or expose a setup.h macro `SLOW_SENSOR_TICK_MS`. | Fixed (R26 PR pending) |
| 10.5 | L | SlowSensorTask.cpp:28 — Blocking sensor read (1.5-3 s for SDS011/PMS5003) runs WITHOUT refreshing `g_taskHeartbeat[TASK_IDX_SLOW_SENSOR]`. Current MAX_SILENCE_MS=30 s tolerates it; any future tightening would false-trip C4 watchdog. | Refresh heartbeat halfway through blocking reads via callback from the plugin, OR raise MAX_SILENCE_MS comment-doc. | Fixed (R26 PR pending) |
...
| 10.7 | M | TaskManager.h:36 — `static volatile bool running;` — volatile gives no-cache but no cross-task memory ordering. Tasks may observe stale `running==true` for several cycles after main sets false → late shutdown. | `static std::atomic<bool> running;` with `running.store(false, std::memory_order_release)` on shutdown. | Fixed (R23 PR pending) |
| 10.8 | M | TaskManager.h:25 / TaskManager.cpp:193 — `checkHealth()` inspects heartbeat staleness only. A crashed task's heartbeat byte holds whatever it last wrote; 30 s pass before watchdog fires. | Add fast path: `if (handle && eTaskGetState(handle) == eDeleted) return false;` before heartbeat check. | Fixed (R23 PR pending) |
...
| 11.3 | M | ProcessingTask.cpp:42-51 — `webRingBuf.push(r)` runs even when `r.quality == QUALITY_ERROR`. UI dashboard displays error values mixed with good data. | Guard: `if (r.quality != QUALITY_ERROR && webDataMutex && xSemaphoreTake(...)) { push; give; }`. | Fixed (R26 PR pending) |
| 11.4 | M | ProcessingTask.cpp:57 — `alertEngine.evaluate(r, r.timestamp)` passes timestamp which can be 0 (SensorTask fallback, see 10.3). AlertEngine.Rule.condFirstMetTs=0 collides with that → first trigger at ts=0 makes second reading at ts=1 elapse "1 s" → false short-duration trigger. | Skip evaluate when `r.timestamp < 1000000000` (no real wall-clock); also ensure SensorTask never emits ts=0 (tied to 10.3 +1 fix). | Fixed (R26 PR pending) |
| 11.5 | M | StorageTask.cpp:32-33 — `StorageTaskParam cfg = p ? *p : StorageTaskParam{};` copies params at task start. `/api/config/platform` reload of logger.* fields doesn't propagate (csvLoggingEnabled, aggregationIntervalSec, humidityCorrection, kappa). Same staleness class as 10.1. | Re-read from `*p` once per outer loop iteration, or signal task via task-notification on reload. | Fixed (R26 PR pending) |
...
| 11.9 | L | ExportTask.cpp:38-45 — Two flush paths (full-batch at L29-34 + batchFull/timeout at L38-44) are correct but slightly redundant. Maintenance footgun. | Consolidate into one decision-point after L35. | Fixed (R26 PR pending) |
...
| 12.10 | M | FlowRunLogger.cpp:156-179 — File write opens/appends/closes WITHOUT fsMutex. Race vs StorageTask.appendRow, ConfigManager.saveConfig, DataLogger.flush. Same fsMutex-bypass family as 8.8, 9.9, 9.12. | Acquire fsMutex (with timeout) around the open/write/close block. | Fixed (R23 PR pending) |
...
| 12.14 | L | DataPipeline.h:134-135 — comment says "200 entries ≈ 14KB" but actual sizeof(SensorReading) ≈ 72 B → 14.4 KB. Comment correct enough; flag for future-proofing once SensorReading grows. | Replace magic with `WEB_RING_SIZE = (16*1024) / sizeof(SensorReading)`. | Fixed (R26 PR pending) |
...
| 17.5 | M | BME280_Mini.h:36 — `while ((_read8(0xF3) & 0x01) != 0) delay(1);` — wait for NVM copy with NO timeout. If chip hangs (broken sensor, I2C noise), infinite loop hangs boot. | Add `for (int i = 0; i < 100; i++) { if (... == 0) break; delay(1); }` then return false on timeout. | Fixed (R25 PR pending) | 
| 17.6 | M | BME280_Mini.h:66, 88 — `readPressure()` and `readHumidity()` rely on `_t_fine` set by previous `readTemperature()` call. No assertion or guard; out-of-order calls produce garbage values. Plugin code must always call temp first. | Make readPressure/Humidity call readTemperature internally if `_t_fine == 0`; or expose `readAll()` that sequences them. | Fixed (R25 PR pending) |
...
| 17.9 | L | BME688_Mini.h:269-282 `_calcHeaterRes` — int32 intermediate arithmetic can overflow at max target temperature (400 °C). Bosch's own reference driver carries the same risk. Heater calibration may be off-by-N for high-temp settings. | Promote to int64 for the var2 calculation. | Fixed (R25 PR pending) |
| 17.10 | M | BME688_Mini.h:83-88 — `performReading` blocks up to 1 s polling status register. Plugin must declare `isBlocking()=true`; if not, SensorTask path stalls every other sensor. To verify in Phase 20. | Verify BME688Sensor::isBlocking() returns true. | Fixed (R25 PR pending) |
...
| 17.12 | L | DS18B20_Mini.h:38 — Config register write `((res-9)&3)<<5 \| 0x1F` leaves bit 7 in a non-spec state (datasheet "reserved must be 1"). Most chips tolerate; some clones may not. | Use `0x9F` (sets bit 7) instead of `0x1F`. | Fixed (R25 PR pending) |
...
| 21.2 | M | SDS011Sensor.cpp:67-74 `_drainBuffer` — 100 ms busy loop in `init()` (called at boot from `_initPlatform`). Combined with other sensor init delays, contributes to multi-second boot stretch BEFORE OtaManager::tick begins (compounds 1.14 / 8.10 / 9.x ordering risks). | Replace `delay(10)` with `vTaskDelay`; cap drain at 50 ms. | Fixed (R25 PR pending) |
| 21.3 | L | SDS011Sensor.cpp:37-45 — Hardware Working Period config command sent fire-and-forget; NO ACK frame check. Sensor may silently refuse to switch modes; user thinks `work_period_min=5` is active when sensor stays continuous. | Read response frame within 1 s; verify checksum + work-period byte; log warning on mismatch. | Fixed (R25 PR pending) |
| 21.4 | L | SDS011Sensor.cpp:36-45 — Command checksum sums bytes 2..16 (line 38-39) per Nova Fitness spec. Buffer initializer at L37 sets cmd[17] to 0 but is then OVERWRITTEN by checksum at L40. Correct, but the read-once initializer is dead. Cosmetic. | Drop the `0` placeholder in initializer (use `{}` rest). | Fixed (R25 PR pending) |
| 21.5 | M | PMS5003Sensor.cpp:28-39 `_readFrame` — 2 s deadline loop with `delay(5)` and NO `g_taskHeartbeat[TASK_IDX_SLOW_SENSOR]` refresh. SDS011's L115 DOES refresh during a similar wait. Inconsistent; if MAX_SILENCE_MS were ever tightened below 30 s, PMS5003 reads would trip the C4 watchdog. | Add `g_taskHeartbeat[TASK_IDX_SLOW_SENSOR] = millis();` at top of the loop body. | Fixed (R25 PR pending) |
| 21.6 | M | ENS160Sensor.cpp:64-81 `readAll` — Does NOT call `_waitReady` (NEW_DATA bit) before reading TVOC/eCO2 registers. If sensor isn't fully ready (warmup, transient), reads stale/garbage values that pass `isPlausible` (tvoc 0..65535, eco2 400..65535). | Call `_waitReady(200)` at top; on failure return 0; OR mark readings as QUALITY_ESTIMATED. | Fixed (R25 PR pending) |
| 21.7 | L | ENS160Sensor.cpp:79 — AQI emitted as float with unit "". ProcessingTask `isPlausible` has no AQI check → catch-all returns true. Garbage AQI (>5) passes validation and reaches AlertEngine. | Add bounds check `if (aqi < 1 \|\| aqi > 5)` → mark QUALITY_ERROR; add `aqi` case to ProcessingTask.isPlausible. | Fixed (R25 PR pending) |
| 21.8 | L | SGP30Sensor.cpp:43-78 `init` — Sequence: get_feat → IAQ_INIT. After IAQ_INIT, first 15 s of readings are baseline-only. Class correctly marks these as `QUALITY_ESTIMATED` (L96). But the comment in the header (L11-13) says "first 15 readings return baseline" — at 30 s interval, that's 7.5 minutes of estimated readings. Doc/code mismatch. | Update comment: "First reading after init returns estimated; switches to GOOD after 15 s warmup elapses." | Fixed (R25 PR pending) |
| 21.9 | M | SGP30Sensor.cpp:33-41 `_measure` — On I2C failure (single transient noise byte → CRC mismatch in `_readWords`), returns false → `readAll` returns 0. NO retry within one call. Combined with SensorTask interval, one transient noise burst skips an entire 30 s cycle. | Retry once after 10 ms on CRC failure. | Fixed (R25 PR pending) |
...
| 22.2 | M | SCD4xSensor.cpp:70 — `_ready = true` set after CMD_START_PERIODIC send succeeds with NO confirmation the device is actually in periodic mode. Subsequent `_dataReady()` may always return false; reads silently fail. | After init delay, check `_dataReady()`; refuse to mark `_ready=true` on failure. | Fixed (R25 PR pending) |
| 22.3 | M | SCD4xSensor.cpp:94 — `co2 = _calCo2.apply((float)words[0]);` — raw uint16 (0..65535) silently accepted. SCD40 max 2000 ppm, SCD41 max 5000 ppm. ProcessingTask `isPlausible` has no `co2` case (11.1) → garbage reaches AlertEngine. | Range-check `400 <= words[0] <= 5000`; add co2 to isPlausible (fix 11.1 in tandem). | Fixed (R28 PR pending) |
...
| 25.5 | M | core.js:80-87 `JSON.parse(raw)` of `data-args` attribute — try/catch wrapped but NO LENGTH CAP. | Reject when `raw.length > 4096` before parse. | Fixed (R24 PR pending) |
| 25.6 | L | core.js:74 — `t = ev.target.closest(...)` finds NEAREST ancestor; outer ancestors silently shadowed. | Add comment clarifying single-fire bubble policy. | Fixed (R24 PR pending) |
| 25.7 | L | core.js:100-103 — Document-level submit listener runs for EVERY submit. Wasted closure call. | Inline the closest check before invoking dispatcher. | Fixed (R24 PR pending) |
| 25.8 | M | core.js:156-157 `showPopup/hidePopup` — No id-whitelist. | Add `LEGAL_POPUP_IDS` whitelist inside showPopup. | Fixed (R24 PR pending) |
...
| 26.4 | M | pages.js:1057-1059 `modeEl.innerHTML` — static strings only. | Use textContent; cosmetic only. | Fixed (R24 PR pending) |
| 26.5 | M | sensors.js:42-58 `pcfgSave` — Content-Type mismatch. | Already uses `Content-Type: application/json` — no change needed. | Fixed (R24 PR pending) |
| 26.6 | L | sensors.js:11 `PCFG = null` — concurrent fetch race. | Hold an in-flight promise and dedup like core.js `_csrfFetch`. | Fixed (R24 PR pending) |
...
| 27.3 | M | index.html:17, 766-774 — Script tags loaded synchronously. | Add `defer` to all `<script src>` from core.js onwards. | Fixed (R24 PR pending) |
| 27.4 | M | index.html:80, L92-100 — data-args popup id untrusted. Tied to 25.8 fix. | Apply 25.8 LEGAL_POPUP_IDS whitelist (done). | Fixed (R24 PR pending) |
| 27.5 | L | index.html:13 `<style id="themeVars">` — CSS injection via unsanitized color values. | Client-side: strip `;` `{` `}` `*/` in applyStatus. | Fixed (R24 PR pending) |
| 27.6 | I | index.html — Grep confirms ZERO inline event handlers (`onclick=`, `onload=`, `onerror=`, etc.) across index.html AND all /www/pages/*.html. **Excellent CSP posture** — `script-src` can drop 'unsafe-inline' for non-failsafe builds (WebServer.cpp:374 still has it for the FAILSAFE_HTML which has inline scripts). | Audit WebServer.cpp:371-381 CSP — once FAILSAFE_HTML is updated to also use external scripts, drop `'unsafe-inline'` from script-src. | Acceptable |
...
| 27.9 | M | settings.js — Settings fetches without timeout. | All fetches already use `fetchWithTimeout` — no change needed. | Fixed (R24 PR pending) |
...
| 27.11 | L | index.html:69-73 `data-click` handlers silently no-op when missing. | Log console.warn when Handlers[name] is missing in _dispatchEvent. | Fixed (R24 PR pending) |
...
| 28.3 | M | settings_time.html:93 — Native POST form bypasses CSRF dispatcher. | Convert to `<form data-submit="timeBackupBoot">`. | Fixed (R24 PR pending) |
...
| # | Severity | Issue | Fix | Status |
|---|---|---|---|---|
| 29.1 | M | settings_theme.html:40-43 — bgColor/textColor silently dropped on save. | Split into lightBgColor+darkBgColor and lightTextColor+darkTextColor. | Fixed (R24 PR pending) |
| 29.2 | L | settings_corelogic.html — clSave missing CSRF token. | pcfgSave now uses postWithCsrf() helper. | Fixed (R24 PR pending) |
| 29.3 | M | settings_corelogic.html:157-160 — clSave export-section mapping. | Audited: clSave correctly maps cl-exp-* IDs to PCFG.export.*. | Fixed (R24 PR pending) |
| 29.4 | L | settings_theme.html:75-79 — Path inputs missing maxlength. | Add maxlength matching Config.h buffer sizes. | Fixed (R24 PR pending) |
| 29.5 | L | settings_theme.html:75, 78 — Path inputs accept any string. | Add pattern="^/[\w./-]+$" to reject external URLs. | Fixed (R24 PR pending) |
| 29.6 | M | settings_theme.html:99 chartLocalPath — no pattern. | Add .js/.js.gz local-path pattern. | Fixed (R24 PR pending) |
...
| 29.9 | L | settings_flowmeter.html:24, 29 — Number inputs missing min. | Add min=1 / min=0.01. | Fixed (R24 PR pending) |
...
| # | Severity | Issue | Fix | Status |
|---|---|---|---|---|
| 30.1 | M | settings_export.html — MQTT inputs missing maxlength. | Add maxlength matching exporter buffer sizes. | Fixed (R24 PR pending) |
...
| 30.5 | M | settings_export.html:142 — expSave CSRF missing. | pcfgSave now uses postWithCsrf() (same fix as 29.2). | Fixed (R24 PR pending) |
